...
@section('seo')
    <?php
    $data_seo = array(
        'title' => 'Favorite product - '.Helpers::get_setting('seo_title'),
        'keywords' => Helpers::get_setting('seo_keyword'),
        'description' => Helpers::get_setting('seo_description'),
        'og_title' => 'Favorite product - '.Helpers::get_setting('seo_title'),
        ...
        <table class="table tbl-my-reviews">
            ...
            @foreach($data as $row)
                <tr>
                    ...
                    <td>
                        <a href="{{route('single.detail', [$row->product->categories->categorySlug, $row->product->slug])}}">
                            {{ $row->product->title }}
                        </a>
                    </td>
                    <td>
