jQuery.fn.extend({
  IconSelectBox: function ( img ) {
    ...
    var createElements = function () {
      ...
      div_search.classList = btn_select +'-search';

      $(div).append(ul);

      $(div_search).insertBefore(ul);
      //$(div).append(div_search);

      //$(div).append(ul);
      //$(ul).insertAfter(div_search);
    ...
    //var height_element=$(prefix_class +'_b').outerHeight();
    //console.log('Height:'+height_element);
    var addCss = function () {
      ...
      const css = prefix_class + '-a {' +
                      ...
                      'border-bottom: 1px solid #DCD6D6;'+
                      '}'+
                      prefix_class + '-search .search-box-selected {' +
                      'width: 100%;' +
                      'height: 30px;'+
                      'line-height: 30px;'+
                      'border-radius: 5px;'+
    ...
    var input_search='<input type="text" class="search-box-selected" name="search-box-selected" placeholder="Search.." value="">';
    var item_input = '<div class="search_box">'+input_search+'</div>';
    //optionArray.push(item_input);
    createElements ();
    addCss();
    
    //console.log(optionArray);
    $(prefix_class +'-search').html(input_search);
    ...
    $(prefix_class +'-select-box').click(function(){
      //var box = document.querySelector(prefix_class +'_b');
      ...
      //var height_box=$(prefix_class).outerHeight();
      //console.log(height_box);
      //$(prefix_class +'-b').css({'height':height_box});
      $(prefix_class +'-b').toggle();
    });

    $(document).on('keyup', prefix_class+'-search .search-box-selected', function() {
       var $val = $(this).val().trim(),
           $select = $(prefix_class+'-a');
            // Check if the input isn't empty
       if ($val != '') {
              $select.children('li').addClass('hide')
              $select.children('li').filter(function(e) {
                return $(this).text().toLowerCase().search($val.toLowerCase()) != -1;
              }).removeClass('hide');
       } else {
         $select.children().removeClass('hide');
       }
